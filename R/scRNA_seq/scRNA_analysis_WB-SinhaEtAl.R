...
data_dir = "natMed_scRNA/scRNA_analysis/data/"
out_dir = "natMed_scRNA/scRNA_analysis/out_blueprint_20220927/"
...
if( length(sobj_list_Rds) == 0 ) {
  ...
  samplename <- filenames[1] %>% str_extract("C\\d+")
  ...
  for (file in filenames[2:length(filenames)]) {
    samplename <- file %>% str_extract("C\\d+")
  ...
  covid_sobj$dex <- ifelse( covid_sobj$patient %in% paste0("C", 1:9) , FALSE, TRUE )
...
if( length(processed_Rdata) == 0 ) {
  ...
  cutoffs$nFeature_RNA.lower = c( rep(0, length(samplenames)) )  # After droping HS450
  cutoffs$nFeature_RNA.upper = c( rep(8000000000000, length(samplenames)) )
  #cutoffs$percent.mt.upper = c( rep(15,8), 25, rep(15,13) ) # With HS450
  cutoffs$percent.mt.upper = c( rep(1000, length(samplenames)) ) # After droping HS450
