...
data_dir = "ETA_scRNA/scRNA_analysis/data/"
out_dir = "ETA_scRNA/scRNA_analysis/out_blueprint_20220927/"
gsea_dir = "gsea_pathways"
# Read dex_tp_data from the object Lucile have generated.
data = readRDS(file.path(data_dir,"dex_tp_data_unfiltered_with_impacc_status_20220527_withVentDaysETA.RDS"))

## Added "MVIR1-HS233-D0","MVIR1-HS72-D4","MVIR1-HS414-D2" samples which were not part of the analysis Aartik performed.
## Missing patient_data for MVIR1-HS450, so it will will excluded from the DGE analysis.
#eta_samples <- c("MVIR1-HS233-D0","MVIR1-HS72-D4","MVIR1-HS414-D2","MVIR1-HS1-D1","MVIR1-HS2-D1","MVIR1-HS24-D0","MVIR1-HS38-D0","MVIR1-HS50-D0","MVIR1-HS51-D1","MVIR1-HS89-D1","MVIR1-HS107-D0","MVIR1-HS145-D2","MVIR1-HS158-D4","MVIR1-HS161-D4","MVIR1-HS340-D1","MVIR1-HS357-D2","MVIR1-HS389-D2","MVIR1-HS398-D4","MVIR1-HS403-D2","MVIR1-HS410-D1","MVIR1-HS415-D4","MVIR1-HS450-D4")
# Latest
eta_samples <- c("MVIR1-HS72-D4","MVIR1-HS414-D2","MVIR1-HS1-D1","MVIR1-HS2-D1","MVIR1-HS24-D0","MVIR1-HS50-D0","MVIR1-HS89-D1","MVIR1-HS107-D0","MVIR1-HS161-D4","MVIR1-HS340-D1","MVIR1-HS357-D2","MVIR1-HS389-D2","MVIR1-HS398-D4","MVIR1-HS403-D2","MVIR1-HS410-D1","MVIR1-HS415-D4","MVIR1-HS471-D2")
data <- data %>%
  dplyr::filter(timepoint %in% eta_samples & data_type == "EA_scRNASeq")



# Load patient data
patient_data <- readRDS( file.path(data_dir, "df_metadata_20220527.RDS"))
if(FALSE){
  patient_data_one <- matrix(NA, nrow=1, ncol=ncol(patient_data)) %>% data.frame() %>% setNames(names(patient_data)) %>% 'rownames<-'("MVIR1-HS450")
  patient_data_one$record_id <- 1450
  patient_data_one$age <- 49
  patient_data_one$sex_at_birth <- "Female"
  patient_data <- rbind(patient_data, patient_data_one)
}
# Add dex overlap status to patient data
patient_data_wOverlapStatus <- inner_join(
  patient_data %>% 
    rownames_to_column("patient"), 
  data %>% 
    dplyr::select(patient, timepoint, dex, overlap, days_bw_sampling_ventstart) %>% 
    ## MVIR1-HS168-D0 has both TRUE and FALSE overlap status. So removing one of them
    #dplyr::filter(! (timepoint == "MVIR1-HS168-D0" & overlap == TRUE) ) %>%
    mutate(overlap = ifelse(is.na(overlap), FALSE, overlap)) %>% 
    # "overlap == FALSE & dex == TRUE" is not allowed (ETA was collected on the same day for HS233 as the dex start date. But since the ETA collection time isn't available, we can't say if the sample was collected before or after dex. Therefore, removing the samples like HS233 using this filter).
    # But if there is genuine sample for which we know the sample was collected before dex treatment in a patient, this filter should be amended.
    mutate(overlap = ifelse(overlap == FALSE & dex == TRUE, NA, overlap) ) %>%
    unique(), 
  by="patient") %>%
...
if( length(sobj_list_Rds) == 0 ) {
  ...
  for (file in filenames[2:length(filenames)]) {
    samplename <- file %>% str_extract("MVIR1-HS[:digit:]+-D\\d+")
...
if( length(processed_Rdata) == 0 ) {
  ...
  cutoffs$nCount_RNA.lower = c( rep(0, length(samplenames)) )
  cutoffs$nCount_RNA.upper = c( rep(50000, length(samplenames)) )
  #cutoffs$nFeature_RNA.lower = c( rep(150,8), 200, 200, rep(150,4), 200, rep(150,2), 200, rep(150,3), 200 )  # With HS450
  cutoffs$nFeature_RNA.lower = c( rep(150,5), 200, rep(150,11) )  # After droping HS450
  cutoffs$nFeature_RNA.upper = c( rep(8000, length(samplenames)) )
  #cutoffs$percent.mt.upper = c( rep(15,8), 25, rep(15,13) ) # With HS450
  cutoffs$percent.mt.upper = c( rep(15,5), 25, rep(15,11) ) # After droping HS450
  cutoffs$percent.ribo.upper = c( rep(60, length(samplenames)) )
