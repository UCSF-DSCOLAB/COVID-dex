...
data_dir = "WB_scRNA/scRNA_analysis/data/"
out_dir = "WB_scRNA/scRNA_analysis/out_blueprint_20220927/"
gsea_dir = "gsea_pathways"
# Read dex_tp_data from the object Lucile have generated.
data = readRDS(file.path(data_dir,"dex_tp_data_unfiltered_with_impacc_status_20220527_withVentDaysWB.RDS"))

# Latest
wb_samples <- c("MVIR1-HS1-D0","MVIR1-HS2-D0","MVIR1-HS50-D0","MVIR1-HS402-D0","MVIR1-HS403-D0","MVIR1-HS405-D0","MVIR1-HS412-D0","MVIR1-HS414-D0","MVIR1-HS415-D0","MVIR1-HS427-D0")
data <- data %>%
  dplyr::filter(timepoint %in% wb_samples & data_type == "WB_scRNASeq")
...
patient_data_wOverlapStatus <- inner_join(
  ...
  data %>% 
    ...
    # "overlap == FALSE & dex == TRUE" is not allowed (Not applicable for WB, but ETA was collected on the same day for HS233 as the dex start date. But since the ETA collection time isn't available, we can't say if the sample was collected before or after dex. Therefore, removing the samples like HS233 using this filter).
...
if( length(sobj_list_Rds) == 0 ) {
  ...
  # Select files for wb_samples
  timepoint_from_filenames <- as.vector(str_match(filenames, "MVIR1-HS\\d+-D\\d+"))
  filenames <- filenames[ timepoint_from_filenames %in% wb_samples ]
  ...
  if(! samplename %in% c("MVIR1-HS1-D0","MVIR1-HS2-D0","MVIR1-HS50-D0"))
  ...
  covid_sobj$doubletFinderCalls[ is.na(covid_sobj$doubletFinderCalls) ] = "Singlet"
...
if( length(processed_Rdata) == 0 ) {
  ...
  cutoffs$nFeature_RNA.lower = c( 200, 200, 150, 150, 200, 150, 200, rep(150,6), 120)
  cutoffs$nFeature_RNA.upper = c( rep(8000, length(samplenames)) )
  cutoffs$percent.mt.upper = c( rep(15, length(samplenames)) )
  ...
  # For 10 file datasets
  cutoffs = data.frame(samplename = samplenames)
  cutoffs$nCount_RNA.lower = c( rep(0, length(samplenames)) )
  cutoffs$nCount_RNA.upper = c( rep(50000, length(samplenames)) )
  cutoffs$nFeature_RNA.lower = c( rep(150, length(samplenames)) ) # c( 200, 200, 100, 150, 200, 200, rep(150,5))
  cutoffs$nFeature_RNA.upper = c( rep(8000, length(samplenames)) )
  cutoffs$percent.mt.upper = c( rep(15, length(samplenames)) )
  cutoffs$percent.ribo.upper = c( rep(60, length(samplenames)) )
  cutoffs$percent.mt.lower = c( rep(0, length(samplenames)) )
  ...
  #covid.anchors = readRDS("COVIDintegrationanchors_2022-04-21.Rds")
  ...
  rm(covid_sobj.list)
  gc()
  #covid.combined = readRDS("covid.combined_2022-04-22.Rds")
